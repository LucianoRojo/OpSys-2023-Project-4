...
bool file_basename_valid(const u8 base_name[8]) {
    unsigned i;
    if (base_name[0] == '\0' || base_name[0] == ' ' || base_name[0] == FAT_FILENAME_DELETED_CHAR) {
...
void build_filename(const u8 *src_name_p, const u8 *src_extension_p,
    ...
    int max_length = 8;
    char fslog_name[] = LOG_FILE_BASENAME;
    fslog_name[0] = (char)FAT_FILENAME_DELETED_CHAR;
    char fslog_extension[] = LOG_FILE_EXTENSION;
    ...
    if (strncmp(fslog_name, (char *)src_name_p, 3) == 0 &&
        strncmp(fslog_extension, (char *)src_extension_p, 3) == 0) {
        *dst_name_p = 'f';
        name_len--;
        dst_name_p++;
        src_name_p++;
