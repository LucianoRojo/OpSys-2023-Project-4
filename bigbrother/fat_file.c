...
static fat_file init_file_from_dentry(fat_dir_entry dentry, fat_file parent) {
    ...
    printf("\nCREANDO ARCHIVO %s.%s\n", dentry->base_name, dentry->extension);
...
static bool ignore_dentry(const fat_dir_entry disk_dentry) {
    ...
    bool is_log = is_log_file_dentry(disk_dentry->base_name,disk_dentry->extension);
    bool b =  !is_log && ((disk_dentry->attribs & (FILE_ATTRIBUTE_VOLUME)) ||
...
void fat_file_unlink(fat_file file, fat_file parent) {
    u32 first_cluster = 0, next_cluster = 0;

    first_cluster = file->start_cluster;

    // Mark following clusters as not used
    fat_table_set_next_cluster(file->table, first_cluster, FAT_CLUSTER_FREE);
...
/* Establece el archivo a "pendiente a eliminar" */
int set_dentry_pending(fat_file file, fat_file parent){
    DEBUG("SETEANDO PENDIENTE A ELIMINAR\n");
    file->dentry->base_name[0] = FAT_FILENAME_DELETED_CHAR;
